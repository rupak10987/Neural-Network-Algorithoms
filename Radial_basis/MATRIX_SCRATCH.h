...
void MATRIX_Transpose(float** A,int num_of_row_A,int num_of_col_A,float** R)
{
...
//for radial basis
void CALC_DISTANCE(float** MAT_A,int num_of_row_A,int num_of_col_A,float** MAT_B,int num_of_row_B,int num_of_col_B,float** result)
{
    if(num_of_col_A!=num_of_row_B)
    {
        std::cout<<"Multiplication operation not possible between mat sizes "<<num_of_row_A<<"X"<<num_of_col_A<<" and "<<num_of_row_B<<"X"<<num_of_col_B<<std::endl;
...
void MATRIX_GAUSSIAN_TRANSFORM(float **MAT_A,int num_of_row_A,int num_of_col_A,int k)
{
for(int i=0; i<num_of_row_A; i++)
    {
        for(int j=0; j<num_of_col_A; j++)
        {
            MAT_A[i][j]=pow(2.718,-1*k*MAT_A[i][j]);
        }
    }
}
